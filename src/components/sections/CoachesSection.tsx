  ...
  return (
    <section className={`py-16 md:py-24 px-4 md:px-6 ${theme === 'dark' ? 'bg-zinc-950' : 'bg-white'}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-5xl mx-auto group/list">
          {[
            ...
            { name: 'Coach Dave', role: 'Speed & Flow Expert', desc: 'Master of efficiency, speed courses, and helping athletes enter flow states on demand.', colSpan: true }
             ...
             <div key={i} className={`${theme === 'dark' ? 'bg-zinc-900 subtractive-border-dark' : 'bg-neutral-50 subtractive-border-light'} border-[1px] p-6 md:p-8 flex flex-col items-center text-center transition-opacity duration-500 group-hover/list:opacity-50 hover:!opacity-100 ${coach.colSpan ? 'md:col-span-2 lg:col-span-1' : ''}`}>
               <div className={`w-32 h-32 ${theme === 'dark' ? 'bg-zinc-800 border-zinc-950' : 'bg-zinc-200 border-white'} border-[1px] mb-6 overflow-hidden relative flex items-center justify-center`}>
